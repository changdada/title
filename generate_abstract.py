...
def predict_one_abstract(model, tokenizer, device, args, content):
    """
    对单个样本进行预测
    Args:
        model: 模型
...
def main():
    ...
    model.to(device)
    model.eval()
    print('开始对新闻生成摘要，输入CTRL + Z，则退出')
    try:
        while True:
            content = input("输入的新闻正文为:")
            abstract = predict_one_abstract(model, tokenizer, device, args, content)
            print(abstract[2])
